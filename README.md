...
- Pass `--decrypt` if the CDN contents are to be decrypted before packaging. Requires `seeddb.bin` placed in the `lib/resources` folder for 9.6.0-26+ encrypted files.
```py
...
- Note that clean CDN contents are not guaranteed as the CIA may have improper contents (e.g. due to being decrypted, dev-signed)
```py
python3 ntool.py cia2cdn <path_to_cia> (--out <path_to_output_folder>) (--titlekey <titlekey>) (--cia-dev)
```

### Convert CIA to CIA (Decrypt-Encrypt switching)
- Pass `--cia-dev` if the CIA is dev-signed
- Pass `--force-mode` to copy the CIA from one location to another.
```py
python3 ntool.py cia2cia <path_to_cia> (--out <path_to_output_folder>) (--cia-dev) (--force-mode)
```
